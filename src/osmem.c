...
void *os_calloc(size_t nmemb, size_t size)
{
	...
	// void *memory = os_malloc(nmemb * size);
...
block_meta *realloc_simple(block_meta *destination, block_meta *item, size_t size) {
	...
	// memmove(destination, item, destination->size);
...
block_meta *realloc_split(block_meta *item, size_t size) {
	block_meta *fre = (char *)item + size + SIZE_T_SIZE;

	fre->size = item->size - size - SIZE_T_SIZE;
	fre->status = STATUS_FREE;

	fre->next = item->next->next;
	fre->prev = item;
	item->next->next->prev = fre;
	item->next = fre;
